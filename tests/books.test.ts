...
} from '../src/repositories/BooksRepository';
  ...
  return jest.fn().mockImplementation(() => {
    return {
      get: jest.fn((key: string) =>
        Promise.resolve(inMemoryStore.get(key) || null),
      ),
...
const MockedRemoteBookRepository =
  OpenLibraryBooksRepository as jest.MockedClass<
    typeof OpenLibraryBooksRepository
  >;
const MockedLocalBookRepository = DatabaseBooksRepository as jest.MockedClass<
  typeof DatabaseBooksRepository
>;
...
describe('GET /search', () => {
  ...
  it('debería devolver una lista de libros en una búsqueda exitosa (cache miss)', async () => {
    ...
    expect(MockedRemoteBookRepository.prototype.search).toHaveBeenCalledWith(
      'woods',
      undefined,
    );
    expect(MockedRemoteBookRepository.prototype.search).toHaveBeenCalledTimes(
      1,
    );
  });
  ...
  it('debería devolver resultados desde el caché en la segunda llamada (cache hit)', async () => {
    ...
    MockedLocalBookRepository.prototype.getByCachedResult.mockResolvedValue(
      mockBooks,
    );
    ...
    expect(
      MockedLocalBookRepository.prototype.getByCachedResult,
